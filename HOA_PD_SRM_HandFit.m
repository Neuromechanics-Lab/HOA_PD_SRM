...
output = table(New_fit, New_Recon);
...
title("Original Reconstruction" + " R2 = " + num2str(Old_fit(1)) + " VAF = " + num2str(Old_fit(2)))
% xlim([-0.2 1.4]); ylim([-1 1])

%% New Recon
plotij(2,1,2,1); hold on
plot(atime(ind_time),measuredData,'b','LineWidth',2) % raw data
plot(atime(ind_time),New_Recon,'m','LineWidth',2) % Reconstruction
if strcmp(ReconName,"Recon_Agonist") || strcmp(ReconName,"Recon_beta") || strcmp(ReconName,"Recon_Cz") % for SRMs w/ only 3 predictors
...
title("New Reconstruction" + " R2 = " + num2str(New_fit(1)) + " VAF = " + num2str(New_fit(2)))
