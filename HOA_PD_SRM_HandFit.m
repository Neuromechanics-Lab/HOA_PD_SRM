...
% Inputs: 1) dataAv - SRM output table (class = table) 
% Example: load('\\cosmic.bme.emory.edu\labs\ting\shared_ting\Scott\HOA_PD SRM\HOA_PD_SRM_Outputs__19-Mar-2024.mat')
...
% Outputs: New SRM reconstruction with associated R2 and VAF along with a 
% plot of original SRM reconstruction (from dataAv) and plot of
% new SRM reconstruction based on NewGains specified by the user

% Example Command Window Call Sequence: 
% subj = "HOA07"; direc = 270; mag = 10; ReconName = "Recon_Agonist_TotalDual_CoM"; % specify which participant/condition you want
% ind = find(strcmp(dataAv.patient,subj) & dataAv.pertdir_calc_round_deg == direc & dataAv.condition == mag) % Find Index for New Gains
...
% save("X:\ting\shared_ting\Scott\HOA_PD SRM\HOA_PD_SRM_HandFit_Outputs\" + subj + "_" + num2str(direc) + "_" + num2str(mag) + "_" + ReconName + ".mat","output","-v7.3"); % save
...
%% create index and define plotting limits
...
elseif strcmp(ReconName,"Recon_Antagonist")
    ...
    measuredData = antagonist(ind_time); % what the SRM is reconstructing
    Old_Recon = dataAv.Recon_Antagonist(ind_row,:); %SRM reconstruction time series
    Old_Gains = dataAv.Gains_Antag(ind_row,:); %SRM reconstruction gains
    Old_fit = dataAv.fit_antagonist(ind_row,:); %SRM reconstruction accuracy
...
if strcmp(ReconName,"Recon_Agonist") || strcmp(ReconName,"Recon_beta") || strcmp(ReconName,"Recon_Cz") % for SRMs w/ only 3 predictors & 1 delay
...
elseif strcmp(ReconName,"Recon_Agonist_TotalDual_CoM")  || strcmp(ReconName,"Recon_Antagonist") % for SRMs w/ 6 predictors & 2 delays
    plot(atime(ind_time) + Old_Gains(4),...
        predictors(1,:)*Old_Gains(1)+backLev,...
        'g-','LineWidth',1.2) % CoM Acc Component
    plot(atime(ind_time)+Old_Gains(4),...
    ...
    plot(atime(ind_time)+Old_Gains(end),...
        ...
        'r-','LineWidth',1.2) % CoM Acc - ctx
    plot(atime(ind_time)+Old_Gains(end),...
        predictors(5,:)*Old_Gains(6)+backLev,...
        'r.','LineWidth',0.5) % CoM Vel - ctx
    plot(atime(ind_time)+Old_Gains(end),...
        predictors(6,:)*Old_Gains(7)+backLev,...
        'r--','LineWidth',0.5)% CoM Disp - ctx
    legend('Data',ReconName,...
    ...
    ylabel([tag_ag ' EMG']); xlabel('Time (s)');
elseif strcmp(ReconName,"Recon_Agonist_TotalDual_Cz") || strcmp(ReconName,"Recon_Agonist_TotalDual_beta") % for SRMs w/ only 4 predictors & 2 delays
    plot(atime(ind_time) + Old_Gains(4),...
        predictors(1,:)*Old_Gains(1)+backLev,...
        'g-','LineWidth',1.2) % CoM Acc Component
    plot(atime(ind_time)+Old_Gains(4),...
...
elseif strcmp(ReconName,"Recon_Agonist_TotalDual_CoM") || strcmp(ReconName,"Recon_Antagonist") % for SRMs w/ 6 predictors & 2 delays
    plot(atime(ind_time) + New_Gains(4),...
        predictors(1,:)*New_Gains(1)+backLev,...
        'g-','LineWidth',1) % CoM Acc 1 Component
    plot(atime(ind_time)+New_Gains(4),...
        predictors(2,:)*New_Gains(2)+backLev,...
        'g.','LineWidth',1) % CoM Vel 1 Component
    plot(atime(ind_time)+New_Gains(4),...
        predictors(3,:)*New_Gains(3)+backLev,...
        'g--','LineWidth',1) % CoM Disp 1 component
    plot(atime(ind_time)+New_Gains(end),...
        predictors(4,:)*New_Gains(5)+backLev,...
        'r-','LineWidth',1) % CoM Acc 2 component 
    plot(atime(ind_time)+New_Gains(end),...
        predictors(5,:)*New_Gains(6)+backLev,...
        'r.','LineWidth',1) % CoM Vel 2 component
    plot(atime(ind_time)+New_Gains(end),...
        predictors(6,:)*New_Gains(7)+backLev,...
        'r--','LineWidth',1)% CoM Disp 2 component
    legend('Data',ReconName,...
