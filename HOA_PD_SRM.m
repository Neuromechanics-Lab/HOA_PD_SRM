...
% mags = mags(1);
%Group marker ("HOA" or "PD" -- string)
% groups = unique(dataAv.group); 
%Unique subject code (i.e. "HOA02" -- string)
...
clear Residual Gains_Ag  Gains_Antag  Gains_Beta  Gains_Cz  Recon_Beta  Recon_Cz...
    Recon_Agonist Recon_Antagonist  fit_beta  fit_Cz  fit_agonist  fit_antagonist...
    Gains_Ag_Dual_beta  Gains_Ag_TotalDual_beta Recon_Agonist_Dual_beta ...
    Recon_Agonist_TotalDual_beta  fit_agonist_Dual_beta  fit_agonist_TotalDual_beta...
    Gains_Residual_beta  Recon_Residual_beta  fit_residual_beta...
    Gains_Ag_TotalDual_Cz Recon_Agonist_TotalDual_Cz  fit_agonist_TotalDual_Cz...
    Gains_Residual_Cz  Recon_Residual_Cz  fit_residual_Cz Gains_Ag_TotalDual_CoM...
    Recon_Agonist_TotalDual_CoM  fit_agonist_TotalDual_CoM Gains_Residual_CoM...
...
for Participant = participants' % iterate across each participant
    ...
    for direction = direcs' %iterate across each direction
        ...
        for mag = mags' %iterate across each magnitude
            ...
            [xTotal_ag_dual_CoM, eTotalRecon_ag_dual_CoM, fitTotal_ag_dual_CoM] = fitTotalDualSRM_CoM(...
                agonist(ind_time), atime(ind_time), predictorsDual_CoM,...
                X0Dual, subjID, mag, direction);
            if removeBackLev
